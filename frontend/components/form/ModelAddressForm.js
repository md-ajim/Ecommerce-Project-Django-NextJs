...
import ForgotPasswordPage from "../../pages/form/forgot-password";
  
  export default function ModelAddressForm() {
    ...
    return (
      <>
        <div className="flex bg-black flex-wrap gap-3">
          {sizes.map((size) => (
            <Button key={size} className="bg-black p-2  md:block hidden relative rounded-lg  border border-gray-500 text-white" onPress={() => handleOpen(size)}>
      ...
      >
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">Sign In Shopfinity</ModalHeader>
              <ModalBody>
